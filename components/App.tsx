...
type Screen = 'HOME' | 'ADMIN_LOGIN' |
'ADMIN_PANEL' | 'TESTIMONY_ARCHIVE' | 'DREAM_FORM' | 'INSIGHT_FORM' | 'INSIGHT_CONFIRMATION' | 'PROPHET_PORTAL' | 'SERVICE_SELECTION' | 'DREAM_RESULT' | 'MARRIAGE_FORM' | 'MARRIAGE_RESULT';
    ...
    const handleInsightSubmit = useCallback(async (formData: InsightFormData) => {
        ...
        try {
           ...
           const newRequest: ProphecyRequest = {
                ...
                formData: formData,
                chatHistory: 
[{ role: 'model', content: aiResponse, author: 'AI Assistant' }],
...
// --- Marriage Compatibility Flow ---
    const handleMarriageSubmit = useCallback(async (formData: MarriageCompatibilityFormData) => {
        ...
        try {
            const aiResponse = 
await checkMarriageCompatibility(formData, selectedProphet);
...
Yahweh Prophecy Ministry
                        ...
                        <p className="mt-6 text-lg md:text-xl text-gray-300 max-w-3xl mx-auto text-center">
                           Welcome.
Select a prophet below to seek divine counsel or request an interpretation of your dreams.
</p>
...
onCancel={() => setProphetToDelete(null)}
            ...
            <Toast message={toast.message} show={toast.show} />
             {(screen === 'HOME' || screen === 
'TESTIMONY_ARCHIVE') && <Footer quote="For the Spirit of prophecy is the testimony of Jesus."
